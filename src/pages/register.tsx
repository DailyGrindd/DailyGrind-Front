...
export function Register() {
    ...
    const hasShownGoogleToast = useRef(false);
    const navigate = useNavigate();
    const location = useLocation();
    const dispatch = useDispatch<any>();
    const { user, isAuthenticated, error: authError } = useSelector((state: RootState) => state.auth);
    const { register: registerField, handleSubmit, watch, trigger, getValues, formState: { errors }, reset } = useForm<RegisterRequest & { confirmPassword: string }>({
        mode: "onChange"
    });
    ...
    useEffect(() => {
        if (location.state?.fromGoogleLogin && !hasShownGoogleToast.current) {
            setIsGoogleRegister(true);
            toast.info("Completa tu perfil para registrarte con Google", { duration: 5000 });
            hasShownGoogleToast.current = true;
        }
    }, [location.state]);
    ...
    useEffect(() => {
        if (authError && !hasShownError.current) {
            ...
            if (authError !== 'POPUP_CANCELLED') {
                ...
                // Resetear el flag después de mostrar el error
                setTimeout(() => {
    ...
    const handleCancelGoogleRegister = () => {
        ...
        hasShownGoogleToast.current = false;
        
        // Si viene desde login, volver al login
        if (location.state?.fromGoogleLogin) {
            toast.info("Registro con Google cancelado");
            navigate("/login", { replace: true });
        } else {
            // Si se inició desde el botón del registro, volver al formulario normal
            reset(); 
            setCurrentStep(1);
        }
    };
    ...
    const handleNextStep = async () => {
        ...
        if (isValid && currentStep === 1) {
            ...
            try {
                ...
                const availability = await checkAvailability(emailToCheck, formData.displayName);
                ...
                if (availability?.displayName && !availability.displayName.available) {
                    newErrors.displayName = "El nickname ya está en uso";
                }
                ...
                setLoading(false);
                setAvailabilityErrors({ displayName: "Error al verificar disponibilidad" });
            }
        } else if (isValid) {
    ...
    const handleGoogleRegister = async () => {
        try {
            ...
            if (!location.state?.fromGoogleLogin) {
                ...
                // Mostrar toast solo si se inicia desde aquí
                if (!hasShownGoogleToast.current) {
                ...
                <Card className="w-full shadow-lg border-border">
                    <CardHeader className="space-y-2">
                        ...
                        <CardTitle className="text-center">
                            {isGoogleRegister ? "Completa tu Perfil" : "Crear Cuenta"}
                        </CardTitle>
                        <CardDescription className="text-center">Paso {currentStep} de 3</CardDescription>
                        
                        {/* Mostrar botón para cancelar registro con Google */}
                        {isGoogleRegister && (
                            <div className="flex justify-center">
                                <Button
                                    ...
                                    className="text-muted-foreground hover:text-active hover:bg-transparent "
                                >
                                    <ArrowLeft className="h-4 w-4 text-active font-extrabold" />
                                    {location.state?.fromGoogleLogin ? "Volver al login" : "Cancelar registro con Google"}
                                </Button>
                            </div>
                        )}
                        ...
                        <form onSubmit={handleSubmit(
                            ...
                            {currentStep === 1 && (
                                <div className="space-y-4 animate-in fade-in duration-300">
                                    ...
                                    {isGoogleRegister && (
                                        <div className="p-3 border border-primary rounded-lg mb-2 p">
                                            <p className="text-sm text-primary">
                                                <svg className="w-5 h-5 inline mr-4 " viewBox="0 0 24 24" fill="currentColor">
                                                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                                                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                                                    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                                                    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                                                </svg>
                                                Registrandote con Google
                                            </p>
                                        </div>
                                    )}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {availabilityErrors.displayName && <p className="text-red-500 text-sm">{availabilityErrors.displayName}</p>}
                                    </div>
                            ...
                            <>
                                <div className="relative my-4">
                                    ...
                                    <div className="relative flex justify-center">
                                        <span className="bg-card px-2 text-sm text-muted-foreground">O regístrate con</span>
                                    </div>
                                </div>
